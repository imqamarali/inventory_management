...
class Components extends Component
{
    ...
    public function ClassSectionFields1()
    {
        $classes_list = [];
        $classes = Yii::$app->db->createCommand(
            "SELECT DISTINCT class_id as id, classes.class_name  
            FROM students 
        ...
        foreach ($classes as &$class) {
            ...
            $sections = Yii::$app->db->createCommand(
                "SELECT DISTINCT section_id as id, sections.section_name
                FROM students 
                LEFT JOIN sections ON (students.section_id = sections.id)
                WHERE students.class_id = :class_id 
                ORDER BY students.section_id ASC"
            )
        ...
        // foreach ($classes_list as $class) {echo $class['class_name'];}
        // exit;
        $html = '
                <div class="widget-toolbar" style="padding: 8px 10px;width: 13%;">
                    <select class="form-control" data-placeholder="Class" name="class" style="width: 100%;">
                        <option value="">Select Class</option>';

        foreach ($classes_list as $class) {

            $html .= '<option value="' . htmlspecialchars($class['id']) . '" data-sections="' . htmlspecialchars(json_encode($class['sections'])) . '">
        ...
        $html .= '</select>
                ...
                <div class="widget-toolbar" style="padding: 8px 10px;width: 13%;">
                    <select class="form-control" data-placeholder="Section" name="section" style="width: 100%;">
                ...
                <script>
                    ...
                    $("select[name=\'class\']").change(function() {
                        var class_id = $(this).val(); // Get the selected class ID
                        var sectionsDropdown = $("select[name=\'section\']"); // The section dropdown
                        sectionsDropdown.html(\'<option value="">Select Section</option>\');
                        var selectedClassOption = $(this).find(\'option:selected\');
                        var sections = selectedClassOption.data(\'sections\');
    
                        if (sections && sections.length > 0) {
                            sections.forEach(function(section) {
                                sectionsDropdown.append(\'<option value="\' + section.id + \'">\' + section.section_name + \'</option>\');
                            });
                        }
                    });
                </script>';
    ...
    public function ClassSectionFields()
    {
        $classes_list = [];
        $classes = Yii::$app->db->createCommand(
            "SELECT DISTINCT class_id as id, classes.class_name  
        ...
        // foreach ($classes_list as $class) {echo $class['class_name'];}
        // exit;
        $html = '<div class="widget-toolbar" style="padding: 8px 10px;width: 13%;">
                    <select class="chosen-select" data-placeholder="Section" name="section" style="width: 100%;">
                        <option value="">Select Section</option>
                    </select>
                </div>
                <div class="widget-toolbar" style="padding: 8px 10px;width: 13%;">
                    <select class="chosen-select" data-placeholder="Class" name="class" style="width: 100%;">
                        <option value="">Select Class</option>';
    ...
    public function Classes()
    {
        ...
        if ($role_id == 3) {
            // For teachers, get classes they're teaching
            $classes = Yii::$app->db->createCommand(
                ...
                ORDER BY 
                    CASE 
                        WHEN c.class_name REGEXP '^Class [0-9]+$' THEN CAST(SUBSTRING(c.class_name, 7) AS UNSIGNED)
                        ELSE 9999 
                    END ASC,
                    c.class_name ASC"
            )->bindValue(':school_id', $school_id)->queryAll();
        } else {
            // For admins and others, get all classes
            $classes = Yii::$app->db->createCommand(
                "SELECT * FROM classes 
                WHERE school_id = :school_id
                ORDER BY 
                    CASE 
                        WHEN class_name REGEXP '^Class [0-9]+$' THEN CAST(SUBSTRING(class_name, 7) AS UNSIGNED)
                        ELSE 9999 
                    END ASC,
                    class_name ASC"
            )->bindValue(':school_id', $school_id)->queryAll();
        ...
        // Get assigned sections for each class from class_sections mapping
        foreach ($classes as &$class) {
            $class_id = $class['id'];
            $sections = Yii::$app->db->createCommand(
                "SELECT s.id, s.section_name, s.description
                ...
                ORDER BY 
                    CASE 
                        WHEN LENGTH(s.section_name) = 1 THEN s.section_name
                        ELSE CONCAT('ZZZ', s.section_name)
                    END ASC"
            )->bindValues([
                ':class_id' => $class_id,
                ':school_id' => $school_id
    ...
    public function Sections()
    {
        ...
        $sections = Yii::$app->db->createCommand(
            ...
            ORDER BY 
                CASE 
                    WHEN LENGTH(section_name) = 1 THEN section_name
                    ELSE CONCAT('ZZZ', section_name)
                END ASC"
        )->bindValue(':school_id', $school_id)->queryAll();
    ...
    {
        $subjects = Yii::$app->db->createCommand('SELECT * FROM subjects')->queryAll();
    ...
    {
        return Yii::$app->session->get('user_array')['referance'];
    }
    public function AciveSession()
    {
        $active_session = Yii::$app->db->createCommand('SELECT * FROM `session` WHERE is_active=1')->queryOne();
        return $active_session['session_name'];
    }
    public function Session()
    {
        $items = Yii::$app->db->createCommand(
            'SELECT `session_id`, `session_name`, `icon`, `date`, `end_date`, `is_active`,
            `view_permission`, `edit_permission`, `update_permission`,
            `delete_permission` FROM `session`
        '
        )->queryAll();
        $menuMap = [];
        foreach ($items as $item) {
            $menuMap[$item['session_id']] = [
                'id' => $item['session_id'],
                ...
                'view' => (int) $item['view_permission'],
                'view_permission' => (int) $item['view_permission'],
                'edit' => (int)$item['edit_permission'],
                'edit_permission' => (int)$item['edit_permission'],
                'update' => (int) $item['update_permission'],
                'update_permission' => (int) $item['update_permission'],
                'delete' => (int) $item['delete_permission'],
                'delete_permission' => (int) $item['delete_permission']
            ];
        }
